...
<template>
  <div class="page battle-page">
    ...
    <section v-if="store.backendEnabled" class="card empty-state">
      <span aria-hidden="true">◈</span>
      <h2>{{ store.game.coins }}コイン獲得済み</h2>
      <p>現在のバックエンドはタスク完了時のコイン付与まで対応しています。</p>
      <p>バトル実行APIは未提供のため、この画面から攻撃は送信しません。</p>
      <RouterLink class="button button--wide" to="/tasks">クエストへ戻る</RouterLink>
    </section>
        ...
        <aside class="attack-panel">
          <div class="card">
            ...
            <dl>
              <div>
                <dt>基礎威力</dt>
                <dd>{{ selectedItems.reduce((sum, item) => sum + item.power, 0) }}</dd>
              </div>
              <div>
                <dt>連続{{ store.game.streakDays }}日</dt>
                <dd>×1.25</dd>
              </div>
              <div>
                <dt>今日の達成</dt>
                <dd>{{ store.progress.percentage }}%</dd>
              </div>
            </dl>
            <button
              class="button button--attack button--wide"
              type="button"
              :disabled="selectedIds.length === 0 || attacking"
              @click="attack"
            >
